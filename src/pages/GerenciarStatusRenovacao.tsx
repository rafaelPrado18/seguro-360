...
const GerenciarStatusRenovacao = () => {
  const { data: apiStatuses, isLoading } = useRenovacaoStatuses();
  ...
  const [editingStatus, setEditingStatus] = useState<RenovacaoStatus | null>(null);
  ...
  const openEdit = (status: RenovacaoStatus) => {
    ...
    <AppLayout>
      <div className="space-y-6 max-w-3xl">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Gerenciar Status de Renovação</h2>
            <p className="text-sm text-muted-foreground">Configure os estágios do funil de renovações</p>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <Badge variant={status.tipo === "ganho" ? "default" : status.tipo === "perdido" ? "destructive" : "secondary"} className="text-[9px]">
                      {status.tipo === "ativo" ? "Em Andamento" : status.tipo === "ganho" ? "Renovado" : "Perdido"}
                    </Badge>
                    {status.template_id ? (
                ...
                <div className="flex gap-2">
                  {(["ativo", "ganho", "perdido"] as const).map((t) => (
                    <Button
                      ...
                      className="text-xs"
                    >
                      {t === "ativo" ? "Em Andamento" : t === "ganho" ? "Renovado" : "Perdido"}
                    </Button>
                  ))}
                </div>
              </div>
...
export default GerenciarStatusRenovacao;
