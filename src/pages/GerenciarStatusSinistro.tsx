...
import { useSinistroStatuses, useCreateSinistroStatus, useUpdateSinistroStatus, useDeleteSinistroStatus } from "@/hooks/useSinistroStatus";
...
import type { SinistroStatus } from "@/services/sinistroStatusService";
...
const GerenciarStatusSinistro = () => {
  const { data: apiStatuses, isLoading } = useSinistroStatuses();
  const createMutation = useCreateSinistroStatus();
  const updateMutation = useUpdateSinistroStatus();
  const deleteMutation = useDeleteSinistroStatus();
  const { data: templates = [] } = useWhatsAppTemplates();
  const [statuses, setStatuses] = useState<SinistroStatus[]>([]);
  ...
  const [editingStatus, setEditingStatus] = useState<SinistroStatus | null>(null);
  ...
  const openEdit = (status: SinistroStatus) => {
  ...
  const handleSave = () => {
    ...
    if (editingStatus) {
      updateMutation.mutate(
        ...
        {
          onSuccess: () => { setIsDialogOpen(false); toast({ title: "Status atualizado com sucesso!" }); },
  ...
  const handleDelete = (id: string) => {
    deleteMutation.mutate(id, {
      onSuccess: () => { toast({ title: "Status excluído com sucesso!" }); },
      onError: (err) => { toast({ title: "Erro ao excluir status", description: err.message, variant: "destructive" }); },
    });
  };
    ...
    <AppLayout>
      <div className="space-y-6 max-w-3xl">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-foreground">Gerenciar Status de Sinistro</h2>
            <p className="text-sm text-muted-foreground">Configure os estágios do Kanban de sinistros</p>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    <Badge variant={status.tipo === "ganho" ? "default" : status.tipo === "perdido" ? "destructive" : "secondary"} className="text-[9px]">
                      {status.tipo === "ativo" ? "Em Andamento" : status.tipo === "ganho" ? "Concluído" : "Perdido"}
                    </Badge>
                    {status.template_id ? (
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="Ex: Abertura / Agendamento Vistoria"
                ...
                <div className="flex gap-2">
                  {(["ativo", "ganho", "perdido"] as const).map((t) => (
                    <Button
                      ...
                      className="text-xs"
                    >
                      {t === "ativo" ? "Em Andamento" : t === "ganho" ? "Concluído" : "Perdido"}
                    </Button>
                  ))}
                </div>
              </div>
...
export default GerenciarStatusSinistro;
